...
#slave_ip_address=`ifconfig eth1 | grep "inet addr" | cut -d ':' -f 2 | cut -d ' ' -f 1`
...
binlog_file=`ssh root@$master_ip mysql --login-path=root -e "show master status\G;" 2>&1 | grep "mysql-bin" | cut -d ":" -f2`

#obtenemos la posición del log del maestro
position=`ssh root@$master_ip mysql --login-path=root -e "show master status\G;" 2>&1 | grep "Position" | cut -d ":" -f2`
...
check_replication_status
        ...
        echo $DATE "Intendo solucionar el problema en el esclavo principal..." >> $LOG 2>&1
                ...
                check_replication_status
                if [ "$?" -ne "1" ]; then
                        mysql --login-path=root -e "CHANGE MASTER TO MASTER_HOST='$master_ip', MASTER_DELAY=300, MASTER_USER='replication_user',MASTER_PASSWORD='slave', MASTER_PORT=3306, MASTER_LOG_FILE='$binlog_file', MASTER_LOG_POS=$position;"
